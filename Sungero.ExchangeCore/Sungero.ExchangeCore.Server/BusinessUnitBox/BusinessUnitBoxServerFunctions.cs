...
namespace Sungero.ExchangeCore.Server
{
  partial class BusinessUnitBoxFunctions
  {
    ...
    public virtual bool SyncBoxCounterparties()
    {
      try
      {
        ...
        foreach (var counterparty in counterparties)
        {
          ...
          queueItem.Save();
          Logger.DebugFormat("Create queue item for counteparty OrganizationId {0}", counterparty.Organization.OrganizationId);
        }
                 ...
                 .ThenBy(c => c.Organization.IsRoaming))
        {
          ...
          Logger.DebugFormat("Start processing, queue item {0}, organization Id {1}.", queueItem.Id, queueItem.ExternalId);
          Transactions.Execute(
            () =>
            {
          ...
          Logger.DebugFormat("End processing, queue item {0}, organization Id {1}.", queueItem.Id, queueItem.ExternalId);
        }
    ...
    {
      var сounterpartiesWithEmptyFtsId = Parties.Counterparties.GetAll(c => c.ExchangeBoxes.Any(x => Equals(x.Box, _obj) &&
                                                                                                (x.FtsId == null || x.FtsId == string.Empty)));
      foreach (var counterparty in сounterpartiesWithEmptyFtsId)
      {
    ...
    public virtual void UpdateExchangeStatus(Parties.ICounterparty counterparty, NpoComputer.DCX.Common.IContact contact, ICounterpartyQueueItem queueItem)
    {
      ...
      Logger.DebugFormat("Updated exchange status {0} for counteparty Id {1}", exchangeStatus, counterparty.Id);
    }
                                                                      ...
                                                                      ICounterpartyQueueItem queueItem)
    {
      ...
      {
        Logger.DebugFormat("Counterparty for contact {0} with TIN not exists.", contact.Organization.OrganizationId);
      ...
      else
      {
        if (string.IsNullOrWhiteSpace(contact.Organization.Kpp))
        {
          ...
          if (companiesWithSameTinWithoutTrrc.Count == 1)
          {
            ...
            Logger.DebugFormat("Find one counterparty with same TIN, id {0}.", comparedCounterparty.Id);
          }
          ...
          if (!allCompanies.Any() && counterpartiesWithSameTin.Count == 1)
          {
            ...
            Logger.DebugFormat("Find one person with same TIN, id {0}.", comparedCounterparty.Id);
          }
          ...
          if (comparedCounterparty == null)
          {
            if (companiesWithSameTinWithoutTrrc.Any())
            {
              ...
              Logger.DebugFormat("Find many counterparties with same TIN, without TRRC. Organization without TRRC.");
            }
            else
            {
              doubles.AddRange(counterpartiesWithSameTin.Where(p => !Parties.CompanyBases.Is(p)));
              Logger.DebugFormat("Find many counterparties with same TIN. Organization without TRRC.");
            }
          }
        }
        else
        {
          ...
          if (companiesWithSameTinTrrc.Count == 1)
          {
            ...
            Logger.DebugFormat("Find one counterparty with same TIN, TRRC, id {0}.", comparedCounterparty.Id);
          }
          
          // Нашли только организации с другими кпп, без кпп организаций нет.
          if (!companiesWithSameTinTrrc.Any() && !companiesWithSameTinWithoutTrrc.Any())
          {
            comparedCounterparty = Functions.BusinessUnitBox.CreateCounterparty(_obj, contact);
            Logger.DebugFormat("Find one counterparty with same TIN, other TRRC, id {0}. Organization with TRRC.", comparedCounterparty.Id);
          }
          ...
          if (comparedCounterparty == null)
          {
            if (companiesWithSameTinTrrc.Any())
            {
              ...
              Logger.DebugFormat("Find many counterparties with same TIN, TRRC. Organization with TRRC.");
            }
            else
            {
              doubles.AddRange(companiesWithSameTinWithoutTrrc);
              Logger.DebugFormat("Find many counterparties with same TIN, without TRRC. Organization with TRRC.");
            }
          }
        }
      }
      ...
      if (comparedCounterparty != null)
      {
        ...
        Logger.DebugFormat("Compared counterparty {0}, status updated.", comparedCounterparty.Id);
      }
      return doubles;
    }
    ...
    public virtual Parties.ICounterparty CreateCounterparty(NpoComputer.DCX.Common.IContact contact)
    {
      ...
      counterparty.Save();
      Logger.DebugFormat("Created counterparty for contact {0}, id {1}.", contact.Organization.OrganizationId, counterparty.Id);
      return counterparty;
    }
    ...
    /// <summary>
    /// Получить сервис обмена по умолчанию. Установить Synerdocs, если у выбранной НОР ещё нет абонентского ящика для Synerdocs.
    /// </summary>
    ...
    public IExchangeService GetDefaultExchangeService()
    {
      ...
      var exchangeServices = ExchangeServices.GetAll().Where(x => !alreadyUsedServices.Contains(x)).ToList();
      
      return exchangeServices.OrderByDescending(x => Equals(x.ExchangeProvider, Sungero.ExchangeCore.ExchangeService.ExchangeProvider.Synerdocs)).FirstOrDefault();
    }
